...
if($quid==''){
		echo "<script src='js/popup.js'></script><script>toast('区号错误 ');setTimeout(function(){history.go(-1)},4000)</script>";exit;	
}
if ($uid == '') {
		echo "<script src='js/popup.js'></script><script>toast('角色名错误');setTimeout(function(){history.go(-1)},4000)</script>";exit;
		}	
if ($upass == '') {
		echo "<script src='js/popup.js'></script><script>toast('请输入密码');setTimeout(function(){history.go(-1)},4000)</script>";exit;	
		}
	...
	if (!$vipjson[$uid]) {
		echo "<script src='js/popup.js'></script><script>toast('你还不是授权客户');setTimeout(function(){history.go(-1)},4000)</script>";exit;
	} elseif ($vipjson[$uid]['pwd'] != mima($uid,$upass)) {
		echo "<script src='js/popup.js'></script><script>toast('用户密码不匹配');setTimeout(function(){history.go(-1)},4000)</script>";exit;
	}
	if ($vipjson[$uid]['qu'] != $quid) 
	{echo "<script src='js/popup.js'></script><script>toast('授权用户与当前选择大区不匹配');setTimeout(function(){history.go(-1)},4000)</script>";exit;
	}
      ...
      <div class="modal-body">
			<div class="form-group">
                ...
                <div class="form-group">
				 ...
				 <hr/>
  <div>
  <input type='text' value='' name="name" id='uid' class="form-control" placeholder='请输入角色名'>  
